...
# Plot SMDI
axes[0].plot(smdi.valid_time, smdi.isel(latitude=lat_idx, longitude=lon_idx), 
             color='brown', linewidth=0.5)
axes[0].axhline(-2.0, color='red', linestyle='--', alpha=0.5, label='Drought threshold')
axes[0].set_ylabel('SMDI')
axes[0].legend()
axes[0].grid(True, alpha=0.3)
axes[0].set_title(f'Grid cell: {float(smdi.latitude[lat_idx]):.2f}°N, {float(smdi.longitude[lon_idx]):.2f}°E')

# Plot SVDI
axes[1].plot(svdi.valid_time, svdi.isel(latitude=lat_idx, longitude=lon_idx), 
             color='orangered', linewidth=0.5)
axes[1].axhline(1.0, color='red', linestyle='--', alpha=0.5, label='VPD threshold')
axes[1].set_ylabel('SVDI')
axes[1].legend()
axes[1].grid(True, alpha=0.3)

# Highlight compound days
compound_days = compound_mask.isel(latitude=lat_idx, longitude=lon_idx)
axes[2].fill_between(compound_mask.valid_time, 0, 1, 
                      where=compound_days.values, 
                      color='red', alpha=0.3, label='Compound event days')
axes[2].set_ylabel('Compound Event')
...
ax1.bar(annual_metrics.index, annual_metrics['n_compound_days'], 
        alpha=0.6, color='coral', label='Compound days/year')
ax1.set_xlabel('Year', fontsize=12)
ax1.set_ylabel('Compound Days per Year (spatial avg)', fontsize=12, color='coral')
...
im2 = total_events.plot(ax=axes[1], cmap='YlOrRd',
